...
- **FR-006**: O sistema MUST organizar o conteudo do PDF em secoes consistentes, contemplando: (1) identificacao do relatorio, (2) metadados do periodo e solicitante, (3) resumo executivo, (4) detalhes do periodo, e (5) mensagem de estado vazio quando nao houver dados. A hierarquia visual deve seguir criterios mensuraveis: titulo 18pt Bold, subtitulos 14pt Bold, corpo 11pt Regular, margens 50pt, espacamento 16pt — aplicados identicamente a todas as categorias.
- **FR-007**: O sistema MUST fornecer uma mensagem de estado vazio apropriada quando nao houver dados para compor o relatorio, sem exibir artefatos tecnicos ou headers genericos. Esta mensagem substitui o conteudo da secao de detalhes (secao 4) e mantem as demais secoes intactas. Mensagem padrao: "Nao ha dados disponiveis para o periodo selecionado. Selecione um intervalo diferente ou entre em contato com o administrador." Para CSV, o header de empty state deve ser `Observacao` (nao `mensagem`). NOTA: O codigo atual (feature 008) contem a mensagem legada "Nenhum dado encontrado para o periodo selecionado." — a implementacao desta feature deve substitui-la pela nova mensagem.
...
- **FR-010**: O sistema MUST usar nomes de arquivo e rotulos de historico coerentes com o tipo de artefato entregue. Convencao: `relatorio-{categoria-slug}-{data_inicial}-{data_final}.pdf` para documento executivo (PDF); `exportacao-{categoria-slug}-{data_inicial}-{data_final}.zip` para exportacao operacional (CSV). No historico, badge "PDF" (azul) para documento executivo ou "CSV" (cinza) para exportacao operacional ao lado do nome.
- **FR-011**: O sistema MUST exibir o indicador visual "Experimental" / "Beta" na UI durante a fase de deliberacao da feature, restrito APENAS a pagina de relatorios e ao modal de exportacao — NAO no PDF gerado nem nos itens do historico. Este indicador deve ser visivel para todos os perfis de usuario.
...
- **Artefato executivo** / **Documento executivo**: Documento final em PDF voltado a leitura de negocio, com titulo, metadados, resumo executivo e detalhes.
...
- **SC-003**: Os rotulos de formato na UI de exportacao e no historico devem ser suficientemente claros para que usuarios identifiquem, sem ambiguidade, o documento executivo (PDF) e a exportacao operacional (CSV/ZIP). A validacao deste criterio e qualitativa e sera realizada via cenario manual de diferenciacao no quickstart (T041 + T048), com verificacao de que os badges "PDF" (azul) e "CSV" (cinza), os rotulos "Documento Executivo" e "Exportacao Operacional (.zip)", e os prefixos de nome de arquivo (`relatorio-` vs `exportacao-`) comunicam a distincao de forma inequivoca.
